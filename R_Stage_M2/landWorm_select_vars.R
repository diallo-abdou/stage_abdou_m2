...
# for (i in predictor_non_r){ cat(paste0 ("s(",i,")", sep = " + "))}
...
# round(pR2(model),3)
# rs = summary(model)
# P = predict(model, newdata=df_train)
# A = df_train[,var_rep]
# res <- rms::lrm(A  ~ P, x= TRUE, y = TRUE)
# res = res$stats
...
for (i in 1: (length(variables)-1) ) {
  ...
  rm("results_df","predictor","imp", "var_importance")
}

AB_tot_select_glm 

write.xlsx(x =AB_tot_select_glm,
           file = paste0("variables_selections/",var_rep,"_select_",algo,".xlsx"),
...
df_train = AB_tot_train[, c("AB_tot", predictor_deep,LC_levels)]
df_test = AB_tot_test[, c("AB_tot", predictor_deep,LC_levels)]
df_train$LC_ = NULL
df_test$LC_ =NULL
...
for (i in 1:nbr_iter) {
  ...
  var_importance <- as.data.frame(imp$results)
  # Merge the features
  var_importance <- merge_features(var_importance, LC_levels, "LC_")
...
model = gam(AB_tot ~ LC_ + s(Sand) + s(Clay) + s(pH) + s(N) + s(P) + s(K) + s(CN) + s(C_org) + 
              s(tas_mean_6) + s(tas_mean_120) + s(tas_sd_3) + s(tas_sd_6) + s(tas_sd_120) + 
              s(pr_mean_3) + s(pr_mean_120) + s(pr_sd_3) + s(pet_mean_6) + s(pet_sd_6), 
            data = df_train)
...
{ 
  model = gam(AB_tot ~ LC_+ s(tas_mean_120), data = df_train)
...
df_train = BM_tot_train[, c("BM_tot", predictor_deep,LC_levels)]
df_test = BM_tot_test[, c("BM_tot", predictor_deep,LC_levels)]
df_train$LC_ = NULL
df_test$LC_ =NULL
# converssion des factors et function de fusion
{ 
...
model = gam(BM_tot ~ LC_ + s(Sand) + s(Clay) + s(pH) + s(N) + s(P) + s(K) + s(CN) + s(C_org) + 
              s(tas_mean_6) + s(tas_mean_120) + s(tas_sd_3) + s(tas_sd_6) + s(tas_sd_120) + 
              s(pr_mean_3) + s(pr_mean_120) + s(pr_sd_3) + s(pet_mean_6) + s(pet_sd_6), 
            data = df_train)
...
### A FAIRE MANUELLEMENT
{ 
  model = gam(BM_tot ~ LC_  +  s(tas_mean_6), 
              data = df_train)
...
# set.seed(425)
# split <- rsample::initial_split(prodij_clean_short, prop = 0.8, strata = "Richesse_tot")
# Richesse_tot_train <- rsample::training(split)
# Richesse_tot_test <- rsample::testing(split)

df_train = Richesse_tot_train[,c("Richesse_tot",predictor_non_r )]
df_test = Richesse_tot_test[,c("Richesse_tot",predictor_non_r )]
...
for (i in 1: (length(variables)-1) ) {
  ...
  model = glm(Richesse_tot ~ ., data = df_train)
...
## end variables selection for Richesse_tot with glm ##
...
df_train = Richesse_tot_train[, c("Richesse_tot", predictor_deep)]
df_test = Richesse_tot_test[, c("Richesse_tot", predictor_deep)]
...
for (i in 1:(length(variables) - 1)) {
  ...
  model = randomForest(Richesse_tot ~ ., data = df_train)
...
## end variables selection for Richesse_tot with rf ##
...
for (i in 1:(length(variables) - 1)) {
  ...
  model = gbm(Richesse_tot ~ ., data = df_train)
...
## end variables selection for Richesse_tot with gbm ##
...
df_train = Richesse_tot_train[, c("Richesse_tot", predictor_deep,LC_levels)]
df_test = Richesse_tot_test[, c("Richesse_tot", predictor_deep,LC_levels)]
df_train$LC_ = NULL
df_test$LC_ =NULL
# converssion des factors et function de fusion
{ 
...
Richesse_tot_select_ann <- data.frame(
  Algorithms = character(),
  Response_variables = character(),
  R2_adjusted_train = numeric(),
  R2_test = numeric(),
...
for (i in 1:nbr_iter) {
  ...
  Richesse_tot_select_ann = rbind(Richesse_tot_select_ann, results_df)
...
Richesse_tot_select_ann
# Write the results to an Excel file
write.xlsx(x = Richesse_tot_select_ann,
           file = paste0("variables_selections/", var_rep, "_select_", algo, ".xlsx"),
           rowNames = FALSE, overwrite = TRUE)


## end variables selection for Richesse_tot with ann ##
...
model = gam(Richesse_tot ~ LC_ + s(Sand) + s(Clay) + s(pH) + s(N) + s(P) + s(K) + s(CN) + 
              s(C_org) + s(tas_mean_6) + s(tas_mean_120) + s(tas_sd_3) + s(tas_sd_6) + 
              s(tas_sd_120) + s(pr_mean_3) + s(pr_mean_120) + s(pr_sd_3) + s(pet_mean_6) + s(pet_sd_6), 
            data = df_train)
...
### A FAIRE MANUELLEMENT
{ 
  model = gam(Richesse_tot ~ LC_+ s(pH), 
              data = df_train)
...
## end variables selection for Richesse_tot with gam #
