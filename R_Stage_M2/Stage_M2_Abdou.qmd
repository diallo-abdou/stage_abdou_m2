...
execute: 
  cache: true
# quarto render Stage_M2_Abdou.qmd --cache-refresh 
---
...
extraction <- function(nom_col, tif_file_path, df = bdd, conv = 1) {
  ...
  # Transformer les coordonnées GPS en systÃ¨me de coordonnées du raster
...
plot_boxplot <-function(donnee,
  ...
  theme(axis.text = element_text(size = 10),
        ...
        legend.position = "none",  # Cette ligne supprime la lÃƒÂ©gende
...
-   Explorations de l'occurence des espÃ¨ces ( [species.html](species.html){target="_blank"} )

<!-- Exploration des relations entre les espÃ¨ces et les predicteurs -->

-   ACP sur les predicteurs de l'abondance, la biomasse et la richesse ( [ACP])

-   Resultats ( [Réduction et effets des variables](#rãduction-et-effets-des-variables) )
...
# On récupÃ¨re toutes les colonnes qui commencent par **AB_**
colonnes_AB <- grep("^AB_", names(bdd), value = TRUE)



# On supprimme les colonnes AB_ qui ne sont pas des espÃ¨ces dans le calcule
...
# On calcule la richesse en attribiant 1 Ã  chaque colonne si la valeur est différent de 0 et de NA
bdd$Richesse_tot <- 0
...
# Mettre Ã  jour les ligne correspondant dans la bdd 
...
````{=html}
<!--
# SynthÃ¨se du taux de remplissage
...
-   Average annual air temperature (Â°C) = bio1

```{r Temperature,fig.align='center',fig.height=8}
summary(bdd$bio1)
explo_num(nom_col = "bio1", titre = "tempÂ°.")
```

## Precipitation

-   Annual precipitation (kg/mÂ²) = bio12

```{r Precipitation,fig.align='center',fig.height=8}
summary(bdd$bio12)
explo_num(nom_col = "bio12", titre = "PrécipitatÂ°.")
```
...
::: columns
::: {.column width="25%"}
<br/> Colonnes supprimée : *`r soil_sup`*
:::

::: {.column width="75%"}
```{r,echo=FALSE,fig.height=6,fig.width=9,fig.align='right', eval=FALSE}
...
::: columns
::: {.column width="25%"}
<br/> Colonnes supprimmées :*`r climat_sup`*
:::

::: {.column width="75%"}
```{r,echo=FALSE,fig.height=7,fig.width=9,fig.align='right' , eval=FALSE}
...
```{r select var, echo=TRUE , eval=FALSE}
...
# plot(results_AB_tot, scale = "r2", main='RÂ² criteria')
...
-   Selection by RÂ² adj : stable from `r which.max(adjr2_AB_tot)` variables
```{r, echo=FALSE, fig.align='center'}
plot(results_AB_tot, scale = "adjr2",main='RÂ² adj criteria')
```
...
-   Selection by RÂ² adj : stable from `r which.max(adjr2_BM_tot)` variables
```{r, echo=FALSE, fig.align='center'}
plot(results_BM_tot, scale = "adjr2",main='RÂ² adj criteria')
```
...
-   Selection by RÂ² adj : stable from `r which.max(adjr2_Richesse_tot)` variables
```{r, echo=FALSE, fig.align='center'}
plot(results_Richesse_tot, scale = "adjr2",main='RÂ² adj criteria')
```
...
```{r}
ForetAlea <- function(var_rep, df_app, df_valid,mtry =9,ntree= 1000,maxnodes = NULL){
  ...
  # Calcul du RÂ² ajusté pour train
...
```{r imp abundance , eval=FALSE}
# names(data_deep)
df_explo_AB_tot = data_deep[,c("AB_tot","clcm_lvl3",predicteurs )]
df_explo_AB_tot = drop_na(df_explo_AB_tot)
# summary(df_explo_AB_tot)

set.seed(3500)  # Pour rendre les résultats reproductibles
index <- createDataPartition(df_explo_AB_tot$clcm_lvl3, p = 0.8, list = FALSE)

# Séparer les données en ensembles d'entraÃ®nement et de test
df_train_AB_tot <- df_explo_AB_tot[index, ]  # Données d'entraÃ®nement
...
# all_var_AB_tot=ForetAlea(var_rep ="AB_tot", df_app=df_train_AB_tot, df_valid = df_test_AB_tot,mtry =9,ntree= 1000,maxnodes = NULL)
# 
# all_var_AB_tot$RMSE
...
best20_var_AB_tot=ForetAlea(var_rep ="AB_tot", df_app=df_train_AB_tot[,c("AB_tot",Predictors_f)], df_valid = df_test_AB_tot [,c("AB_tot",Predictors_f)],mtry =9,ntree= 1000,maxnodes = NULL)
...
best20_var_AB_tot=ForetAlea(var_rep ="AB_tot", df_app=df_train_AB_tot[,c("AB_tot",Predictors_f[-8])], df_valid = df_test_AB_tot [,c("AB_tot",Predictors_f[-8])],mtry =9,ntree= 1000)
...
# Séparer les données en ensembles d'entraÃ®nement et de test
df_train_BM_tot <- df_explo_BM_tot[index, ]  # Données d'entraÃ®nement
...
# all_var_BM_tot= ForetAlea(var_rep ="BM_tot", df_app=df_train_BM_tot, df_valid = df_test_BM_tot,mtry =9,ntree= 1000)
...
best20_var_BM_tot= ForetAlea(var_rep ="BM_tot", df_app=df_train_BM_tot[, c("BM_tot",Predictors_f)], df_valid = df_test_BM_tot [, c("BM_tot",Predictors_f)],mtry =9,ntree= 1000)
...
# partialPlot(best20_var_BM_tot$model,  df_train_BM_tot[, c("BM_tot",Predictors_f)], "gps_x", plot = TRUE)
...
### sans OS
best20_var_BM_tot= ForetAlea(var_rep ="BM_tot", df_app=df_train_BM_tot[, c("BM_tot",Predictors_f[-8])], df_valid = df_test_BM_tot [, c("BM_tot",Predictors_f[-8])],mtry =9,ntree= 1000)
# varImpPlot(best20_var_BM_tot$model, main = "Biomass") #produce variable importance plot
...
# Séparer les données en ensembles d'entraÃ®nement et de test
df_train_Richesse_tot <- df_explo_Richesse_tot[index, ]  # Données d'entraÃ®nement
...
# all_var_Richesse_tot= ForetAlea(var_rep ="Richesse_tot", df_app=df_train_Richesse_tot, df_valid = df_test_Richesse_tot,mtry =9,ntree= 1000)
...
best20_var_Richesse_tot= ForetAlea(var_rep ="Richesse_tot", df_app=df_train_Richesse_tot[, c("Richesse_tot",Predictors_f)], df_valid = df_test_Richesse_tot [, c("Richesse_tot",Predictors_f)],mtry =9,ntree= 1000)
...
best20_var_Richesse_tot= ForetAlea(var_rep ="Richesse_tot", df_app=df_train_Richesse_tot[, c("Richesse_tot",Predictors_f[-8])], df_valid = df_test_Richesse_tot [, c("Richesse_tot",Predictors_f[-8])],mtry =9,ntree= 1000)
...
```{r re AB_tot,fig.align='center',fig.show='animate' , eval=FALSE}
...
```{r re AB_tot2,fig.align='center' , eval=FALSE}
df_re_AB_tot= data_deep[,c("AB_tot",Predictors_f)]
for (i in names(df_re_AB_tot[,-1])){
  # plot(df_re_AB_tot[,i],df_re_AB_tot[,1], main = paste("AB_tot &",i), xlab = i,ylab="Abundance" )
...
```{r re BM_tot,fig.align='center',fig.show='animate' , eval=FALSE}
...
```{r re BM_tot2,fig.align='center' , eval=FALSE}
df_re_BM_tot= data_deep[,c("BM_tot",Predictors_f)]
for (i in names(df_re_BM_tot[,-1])){
  # plot(df_re_BM_tot[,i],df_re_BM_tot[,1], main = paste("BM_tot &",i), xlab = i,ylab="Biomass" )
  cat(paste("- Biomass &",i,"\n"))
...
```{r re Richesse_tot,fig.align='center',fig.show='animate' , eval=FALSE}
...
```{r re Richesse_tot2,fig.align='center' , eval=FALSE}
df_re_Richesse_tot= data_deep[,c("Richesse_tot",Predictors_f)]
for (i in names(df_re_Richesse_tot[,-1])){
  # plot(df_re_Richesse_tot[,i],df_re_Richesse_tot[,1], main = paste("Richesse_tot &",i), xlab = i,ylab="Richness" )
...
```{r ACP abundance, echo=TRUE, fig.align='center' , eval=FALSE}
...
```{r ACP Biomass, echo=TRUE, fig.align='center' , eval=FALSE}
...
```{r ACP Richness, echo=TRUE, fig.align='center' , eval=FALSE}
...
::: {.column width="50%"}
<p><img src="Results/abundance_dist_train_and_test.png"/></p>

<p><img src="Results/biomass_dist_train_and_test.png"/></p>

<p><img src="Results/richness_dist_train_and_test.png"/></p>
:::
:::
...
RF_df_grid <- expand.grid(ntree = c(100,300,500,700,900,1000,1300,1500,1700,2000),
                       ...
                       maxnodes = c(10 , 20,  30,  40,  50,  60,  70,  80))
```
...
<p><img src="results_tuning/RF_tuning.png"/></p>
...
<p><img src="results_tuning/GBM_tuning.png"/></p>
...
n <- neuralnet(Res.var~ For +Gua +Nag +Nial +Pmo +Viny + Long +Lat + 
                 Prec + Isot+ CaCO3 +N+ P+ Clay + Silt,
               data = df,
               # hidden = c(1),
               linear.output = F,
               lifesign = 'full',
...
<p><img src="models/ANN_default_model.png"/></p>

<p><img src="models/plot_ANN_default_model.png"/></p>
...
#   patience = 10,         # Nombre d'époques sans amélioration avant l'arrÃªt
#   restore_best_weights = TRUE  # Restaure les poids du meilleur modÃ¨le
# )
# 
# 
# # Fit ANN_tune_AB_tot
...
<p><img src="models/ANN_tuning_model.png"/></p>

<p><img src="Results/fig_ANN_tune_BM_tot.png"/></p>
...
ANN_tune_AB_tot_cor = cor(testtarget,ANN_tune_AB_tot_pred)^2 # RÂ²
...
ANN_tune_BM_tot_cor = cor(testtarget,ANN_tune_BM_tot_pred)^2 # RÂ²
...
ANN_tune_Richesse_tot_cor = cor(testtarget,ANN_tune_Richesse_tot_pred)^2 # RÂ²
...
```{r}
subtitle <- sprintf("Abundance : %.2f Â± %.2f ind/mÂ²", mean(data_deep$AB_tot, na.rm = TRUE), sd(data_deep$AB_tot, na.rm = TRUE))
```
...
GLM_AB_tot <- ggplot(GLM_df_AB_tot, aes(x = Observed, y = Predicted)) +
      ...
      labs(subtitle =paste0(" GLM: RÂ² adj (train) = ", round(GLM_result_AB_tot$R_adj_train,2),
                           "; \n RÂ² = ", round(GLM_result_AB_tot$R_adj_test,2),
...
GAM_AB_tot <- ggplot(GAM_df_AB_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" GAM: RÂ² adj (train) = ", round(GAM_result_AB_tot$R_adj_train,2),
                           "; \n RÂ² = ", round(GAM_result_AB_tot$R_adj_test,2),
...
RF_AB_tot <- ggplot(RF_df_AB_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" RF: RÂ² adj (train) = ", round(RF_result_AB_tot$R_adj_train,2), 
                           "; \n RÂ² = ", round(RF_result_AB_tot$R_adj_test,2),
                           "; RMSE = ",  round(RF_result_AB_tot$RMSE^2,2)),
                            x = "Observed valuess", 
                            y = "Predicted values") + 
      theme_classic() 
best_algo_AB_tot = RF_AB_tot
saveRDS(RF_result_AB_tot$model, "cartographie/RF_mod_AB_tot.RDS")
...
GBM_AB_tot <- ggplot(GBM_df_AB_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" GBM: RÂ² adj (train) = ", round(GBM_result_AB_tot$R_adj_train,2),
                           "; \n RÂ² = ", round(GBM_result_AB_tot$R_adj_test,2),
...
ANN_AB_tot <- ggplot(ANN_df_AB_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") +
labs(subtitle =paste0(" ANN: RÂ² adj (train) = ", round(ANN_tune_AB_tot_results$R_adj_train,2),
                           "; \n RÂ² = ", round(ANN_tune_AB_tot_results$R_adj_test,2),
...
```{r}
subtitle <- sprintf("Biomass : %.2f Â± %.2f g/mÂ²", mean(data_deep$BM_tot, na.rm = TRUE), sd(data_deep$BM_tot, na.rm = TRUE))
```
...
GLM_BM_tot <- ggplot(GLM_df_BM_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" GLM: RÂ² adj (train) = ", round(GLM_result_BM_tot$R_adj_train,2),
                           "; \n RÂ² = ", round(GLM_result_BM_tot$R_adj_test,2),
...
GAM_BM_tot <- ggplot(GAM_df_BM_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" GAM: RÂ² adj (train) = ", round(GAM_result_BM_tot$R_adj_train,2),
                           "; \n RÂ² = ", round(GAM_result_BM_tot$R_adj_test,2),
...
RF_BM_tot <- ggplot(RF_df_BM_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" RF: RÂ² adj (train) = ", round(RF_result_BM_tot$R_adj_train,2), 
                           "; \n RÂ² = ", round(RF_result_BM_tot$R_adj_test,2),
                           "; RMSE = ",  round(RF_result_BM_tot$RMSE^2,2)),
                            x = "Observed valuess", 
                            y = "Predicted values") + 
      theme_classic() 
best_algo_BM_tot = RF_BM_tot
saveRDS(RF_result_BM_tot$model, "cartographie/RF_mod_BM_tot.RDS")
...
GBM_BM_tot <- ggplot(GBM_df_BM_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" GBM: RÂ² adj (train) = ", round(GBM_result_BM_tot$R_adj_train,2),
                           "; \n RÂ² = ", round(GBM_result_BM_tot$R_adj_test,2),
...
ANN_BM_tot <- ggplot(ANN_df_BM_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") +
labs(subtitle =paste0(" ANN: RÂ² adj (train) = ", round(ANN_tune_BM_tot_results$R_adj_train,2),
                           "; \n RÂ² = ", round(ANN_tune_BM_tot_results$R_adj_test,2),
...
```{r}
subtitle <- sprintf("Richness : %.2f Â± %.2f", round(mean(data_deep$Richesse_tot, na.rm = TRUE)), round(sd(data_deep$Richesse_tot, na.rm = TRUE)))
```
...
GLM_Richesse_tot <- ggplot(GLM_df_Richesse_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" GLM: RÂ² adj (train) = ", round(GLM_result_Richesse_tot$R_adj_train,2),
                           "; \n RÂ² = ", round(GLM_result_Richesse_tot$R_adj_test,2),
...
GAM_Richesse_tot <- ggplot(GAM_df_Richesse_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" GAM: RÂ² adj (train) = ", round(GAM_result_Richesse_tot$R_adj_train,2),
                           "; \n RÂ² = ", round(GAM_result_Richesse_tot$R_adj_test,2),
...
RF_Richesse_tot <- ggplot(RF_df_Richesse_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" RF: RÂ² adj (train) = ", round(RF_result_Richesse_tot$R_adj_train,2), 
                           "; \n RÂ² = ", round(RF_result_Richesse_tot$R_adj_test,2),
...
# save model
saveRDS(RF_result_Richesse_tot$model, "cartographie/RF_mod_Richesse_tot.RDS")
# identical(RF_result_Richesse_tot$model, test_s_mod, ignore.environment = TRUE)
...
GBM_Richesse_tot <- ggplot(GBM_df_Richesse_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") + 
      labs(subtitle =paste0(" GBM: RÂ² adj (train) = ", round(GBM_result_Richesse_tot$R_adj_train,2),
                           "; \n RÂ² = ", round(GBM_result_Richesse_tot$R_adj_test,2),
...
ANN_Richesse_tot <- ggplot(ANN_df_Richesse_tot, aes(x = Observed, y = Predicted)) +
      geom_point() + # Ajout des points
      geom_smooth(method = "lm", se = TRUE, color = "red") +
labs(subtitle =paste0(" ANN: RÂ² adj (train) = ", round(ANN_tune_Richesse_tot_results$R_adj_train,2),
                           "; \n RÂ² = ", round(ANN_tune_Richesse_tot_results$R_adj_test,2),
...
**- Summary:** *Results Case 1 -\> repeated data* <br/>
...
```{r}
### model **RÂ² adj RF abundance (train, test)**
# AB_tot_train = read.csv2("datas/AB_tot_train.csv")
# AB_tot_test = read.csv2("datas/AB_tot_test.csv")
RF_result_AB_tot = ForetAlea(var_rep ="AB_tot", 
                             df_app=AB_tot_train, 
...
```{r}
#   **RÂ² adj RF kenStone abundance (train, test)**
# AB_tot_train = read.csv2("datas/AB_tot_train.csv")
# AB_tot_test = read.csv2("datas/AB_tot_test.csv")
...
ecobiosoil_results = data.frame(Datas = rep("EcoBioSoil", 15))
ecobiosoil_results = cbind(ecobiosoil_results, all_result)


ecobiosoil_results <- ecobiosoil_results %>%
...
write.csv(x =ecobiosoil_results,file = "Results/ecobiosoil_results.csv", row.names = FALSE)


df = read.csv("Results/ecobiosoil_results.csv")

ecobiosoil_results = df
colnames(ecobiosoil_results) = c("Datas","Algorithms","Response \n variables",
                               "R2_adj \ntrain","R2_ad \ntest","RMSE","MAE")

ecobiosoil_results %>% datatable(options = list(pageLength = 5))

```

## Comparaison entre EcoBiosoil et LandWorm

```{r comp landW and EcoB, cache = FALSE , eval=FALSE}
library(tidyverse)
library(DT)
# ecobiosoil_results

landworm_results = read.csv("Results/landW/landworm_results.csv")
colnames(landworm_results) = c("Datas","Algorithms","Response \n variables",
                               "R2_adj \ntrain","R2_ad \ntest","RMSE","MAE")
...
write.csv(x =landworm_ecobisoil,file = "Results/landworm_ecobisoil2.csv", row.names = FALSE)

landworm_ecobisoil %>% datatable(options = list(pageLength = 5))
...
# Réduction et effets des variables {#rãduction-et-effets-des-variables}
...
#       geom_point() + # Ajout des points
#       geom_smooth(method = "lm", se = TRUE, color = "red") + 
#       labs(title = "After",subtitle =paste0(" RF: RÂ² adj (train) = ", round(RF_result_AB_tot$R_adj_train,2), 
#                            "; \n RÂ² = ", round(RF_result_AB_tot$R_adj_test,2),
#                            "; RMSE = ",  round(RF_result_AB_tot$RMSE^2,2)),
#                             x = "Observed valuess", 
#                             y = "Predicted values") + 
...
# Predictors_f[!Predictors_f %in% names(bdd2)]
AB_tot_df = bdd2[,c("AB_tot",Predictors_f)]
...
# Les effets locaux accumulés décrivent comment les predicteurs influencent en moyenne la prédiction d'un modÃ¨le d'apprentissage automatique: ALE montre comment la prédiction change localement, lorsque les predicteurs varie. Les marques sur l'axe des x indiquent la distribution des predicteurs, montrant la pertinence d'une région pour l'interprétation (peu ou pas de points signifie que nous ne devons pas surinterpréter cette région).
...
#   geom_point() + # Ajout des points
#   geom_smooth(method = "lm", se = TRUE, color = "red") +
#   labs(title = "After",subtitle =paste0(" RF: RÂ² adj (train) = ", round(RF_result_BM_tot$R_adj_train,2), 
#                            "; \n RÂ² = ", round(RF_result_BM_tot$R_adj_test,2),
#                            "; RMSE = ",  round(RF_result_BM_tot$RMSE^2,2)),
#                             x = "Observed valuess", 
#                             y = "Predicted values") + 
...
BM_tot_df = bdd2[,c("BM_tot",Predictors_f)]
...
#   geom_point() + # Ajout des points
#   geom_smooth(method = "lm", se = TRUE, color = "red") +
#   labs(title = "After",subtitle =paste0(" RF: RÂ² adj (train) = ", round(RF_result_Richesse_tot$R_adj_train,2), 
#                            "; \n RÂ² = ", round(RF_result_Richesse_tot$R_adj_test,2),
#                            "; RMSE = ",  round(RF_result_Richesse_tot$RMSE,2)),
#                             x = "Observed valuess", 
#                             y = "Predicted values") + 
...
Richesse_tot_df = bdd2[,c("Richesse_tot",Predictors_f)]
