...
write.csv2(x =bdd_n_rep,file = "datas/Non_repeated_data/bdd_n_rep.csv", row.names = FALSE)
```
...
levels(data_lm$clcm_lvl3)[levels(data_lm$clcm_lvl3) == "Broad-leaved forest"] <- "Mixed forest"
levels(data_lm$clcm_lvl3)[levels(data_lm$clcm_lvl3) == "Coniferous forest"] <- "Mixed forest"
data_lm$clcm_lvl3= as.factor(data_lm$clcm_lvl3)
# summary_df <- as.data.frame(summary(data_lm$clcm_lvl3))
# colnames(summary_df) <- c("Numbers")
# kable(summary_df,padding = 5)
...
GAM <- function(var_rep, df_app, df_valid, family = 'gaussian',method = "REML", interaction = FALSE){
  ...
  if(var_rep == "Richesse_tot"){ 
    if (interaction == FALSE){
  modelgam<-gam(Richesse_tot ~ s(gps_y) + s(gps_x) + s(bio4) + s(bio10) + s(bio7) + s(CaCO3) + s(P) + s(N) + s(gdd10) + s(pH) + s(bio18) + s(hurs_mean) + s(bio5) + s(bio9) + s(bio13) + s(bio15) + s(bio16) + s(bio11) + s(bio6) + clcm_lvl3mf + clcm_lvl3gua + clcm_lvl3ng + clcm_lvl3nial + clcm_lvl3p + clcm_lvl3v ,
        family=family,method = method,data = df_app)
    } else { 
...
ggsave("Results/graphe_AB_tot.png", plot = graphe_AB_tot, dpi = 300,height = 5,width = 8)
...
ggsave("Results/graphe_BM_tot.png", plot = graphe_BM_tot, dpi = 300, height = 5,width = 8)
...
GLM_Richesse_tot_pred <- GLM_result_Richesse_tot$predit^2

GLM_df_Richesse_tot = data.frame(Observed=Richesse_tot_test[,1]^2,Predicted = GLM_Richesse_tot_pred)
...
GLM_Richesse_tot <- ggplot(GLM_df_Richesse_tot, aes(x = Observed, y = Predicted)) +
      ...
      labs(subtitle =paste0(" GLM: R² adj (train) = ", round(GLM_result_Richesse_tot$R_adj_train,2),
                           ...
                           "; \n RMSE (test) = ",  round(GLM_result_Richesse_tot$RMSE^2,2)),
                            x = "Real values", 
                            y = "Predicted values") +
      theme_classic() 
...
GAM_Richesse_tot_pred <- GAM_result_Richesse_tot$predit^2

GAM_df_Richesse_tot = data.frame(Observed=Richesse_tot_test[,1]^2,Predicted = GAM_Richesse_tot_pred)
...
GAM_Richesse_tot <- ggplot(GAM_df_Richesse_tot, aes(x = Observed, y = Predicted)) +
      ...
      labs(subtitle =paste0(" GAM: R² adj (train) = ", round(GAM_result_Richesse_tot$R_adj_train,2),
                           ...
                           "; \n RMSE (test) = ",  round(GAM_result_Richesse_tot$RMSE^2,2)),
                            x = "Real values", 
                            y = "Predicted values") +
      theme_classic() 
...
RF_Richesse_tot_pred <- RF_result_Richesse_tot$predit^2

RF_df_Richesse_tot = data.frame(Observed=Richesse_tot_test[,1]^2,Predicted = RF_Richesse_tot_pred)
...
RF_Richesse_tot <- ggplot(RF_df_Richesse_tot, aes(x = Observed, y = Predicted)) +
      ...
      labs(subtitle =paste0(" RF: R² adj (train) = ", round(RF_result_Richesse_tot$R_adj_train,2), 
                           ...
                           "; \n RMSE (test) = ",  round(RF_result_Richesse_tot$RMSE^2,2)),
                            x = "Real values", 
                            y = "Predicted values") + 
      theme_classic() 
...
GBM_Richesse_tot_pred = GBM_result_Richesse_tot$predit^2

GBM_df_Richesse_tot = data.frame(Observed=Richesse_tot_test[,1]^2,Predicted = GBM_Richesse_tot_pred)
...
GBM_Richesse_tot <- ggplot(GBM_df_Richesse_tot, aes(x = Observed, y = Predicted)) +
      ...
      labs(subtitle =paste0(" GBM: R² adj (train) = ", round(GBM_result_Richesse_tot$R_adj_train,2),
                           ...
                           "; \n RMSE (test) = ",  round(GBM_result_Richesse_tot$RMSE^2,2)),
                            x = "Real values", 
                            y = "Predicted values") +
      theme_classic() 

best_algo_Richesse_tot = GBM_Richesse_tot
...
ANN_Richesse_tot_pred = ANN_tune_Richesse_tot_pred^2

ANN_df_Richesse_tot = data.frame(Observed=Richesse_tot_test[,1]^2,Predicted = ANN_Richesse_tot_pred)
...
labs(subtitle =paste0(" ANN: R² adj (train) = ", round(ANN_tune_Richesse_tot_results$R_adj_train,2),
                           ...
                           "; \n RMSE (test) = ",  round(ANN_tune_Richesse_tot_results$rmse^2,2)),
                            x = "Real values", 
                            y = "Predicted values") +
      theme_classic()
...
ggsave("Results/graphe_Richesse_tot.png", plot = graphe_Richesse_tot, dpi = 300, height = 5,width = 8)
...
ggsave("Results/GBM_Richesse_tot_fig.png", plot = GBM_Richesse_tot_fig, dpi = 300,height = 8)
```
...
**The best algorithm for total Richness is: GBM **
...
```{r}
# comp_graphe_poly = ggpubr::ggarrange(best_algo_AB_tot, best_algo_BM_tot, best_algo_Richesse_tot,
#                           labels = c('(a)', '(b)','(c)'),ncol = 3,
#                           common.legend = TRUE,
#                           legend = 'right'
# )
# ggsave("Results/POLY/comp_graphe_poly.png", plot = comp_graphe_poly, dpi = 300,height = 2.5,width = 8)
```
...
<p align="center">
  <img src="Results/POLY/comp_graphe_poly.png">
</p>

## Explanatory power of variables

-   Abundance (X1 = LC, soil, climate; X2 = Space)
...
df_=drop_na(df)
df_response = df %>% select(all_of(VAR_REP))

df_predictors = df %>% select(-AB_tot, -gps_x,-gps_y)

df_space = df %>% select(gps_x,gps_y)
...
-   Biomass (X1 = LC, soil, climate; X2 = Space)
```{r,fig.align='center',fig.dpi=300}
# BM_tot --------------------------------------------------------------
VAR_REP = "BM_tot"
TITRE = "Biomass"
...
df_predictors = df %>% select(- BM_tot, - gps_x,- gps_y)
...
df_predictors = df %>% select(-Richesse_tot, -gps_x,-gps_y)
