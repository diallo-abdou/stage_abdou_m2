...
# head(result[, c("GPS_X", "GPS_Y")])
# Créer un objet "SpatialPoints" à partir des colonnes GPS_X et GPS_Y du tableau rmqs_description
coords <- SpatialPoints(coords = result[, c("GPS_X", "GPS_Y")], proj4string = src_crs)

# Effectuer la transformation de Lambert 93 vers WGS84 (degrés décimaux)
coords_transformed <- spTransform(coords, dst_crs)

# Mettre à jour les colonnes GPS_X et GPS_Y du tableau rmqs_description avec les nouvelles valeurs transformées
result$GPS_X <- coords_transformed@coords[, 1]
result$GPS_Y <- coords_transformed@coords[, 2]
...
```{r}
variables = names(prodij)[c(99,100,103,104:116, 118:119, 121:122)]
...
-   Je Supprime le **Travail_du_sol** et la **Fertilisation** des variables potentiels
...
```{r}
variables <- variables[!variables %in% c("Texture_GEPPA", "CaCO3_tot", "Fertilisation","Travail_du_sol")]
variables
```
...
variables_factor = c("Details_Milieu_Niv3")
...
**Conclusion: Il reste donc 9 variables explicatives numerique + 1 variables factorielles**
...
ggplot(df, aes(x=y)) +
  ...
  theme_gray() +
  labs(title="Abundance", subtitle =AB_tot_sub,  x="Values", y = "Counts") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(df, aes(x=y)) +
  ...
  geom_density(fill="black", alpha=0.2) +
  theme_gray() +
  labs(title="biomass", subtitle =BM_tot_sub,  x="Values", y = "Counts") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(df, aes(x=y)) +
  ...
  geom_density(fill="black", alpha=0.2) +
  theme_gray() +
  labs(title="richness", subtitle =Richesse_sub,  x="Values", y = "Counts") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
variables_factor = c("OS","Ferti_","w_sol_")
vars_scale = predictor_deep[!predictor_deep %in% variables_factor]
# prodij_explo[,vars_scale] = scale(prodij_explo[,vars_scale])
...
<br/> <br/> **Toutes les variables explicatives: =\> RG, GBM et ANN**
...
<br/> **Aucune corrélation: =\> GLM et GAM** (\<0.7) <br/>
...
5.  **Calcul des métriques de performance**
    ...
    -   `RMSE`, `R² ajusté` pour l'entraînement, `R²` pour le test, et `MAE`.
...
```{r modeling BM_tot}
# BM_tot --------------------------------------------------------------------------
df_mod_BM_tot = data_deep[,c("BM_tot",predictor_deep)]
...
biomass_dist_train = ggplot(df, aes(x=y)) +
  ...
  geom_density(fill="black", alpha=0.2) +
  theme_gray() +
  labs(title="Biomass: Train",  x="Values", y = "Counts") +
  theme(plot.title = element_text(hjust = 0.5))
...
biomass_dist_test = ggplot(df, aes(x=y)) +
  ...
  geom_density(fill="black", alpha=0.2) +
  theme_gray() +
  labs(title="Biomass: Test",  x="Values", y = "Counts") +
  theme(plot.title = element_text(hjust = 0.5))
...
```{r modeling Richesse}
# Richesse --------------------------------------------------------------------------
df_mod_Richesse = data_deep[,c("Richesse",predictor_deep)]
...
richness_dist_train = ggplot(df, aes(x=y)) +
  ...
  geom_density(fill="black", alpha=0.2) +
  theme_gray() +
  labs(title="Richness: Train",  x="Values", y = "Counts") +
  theme(plot.title = element_text(hjust = 0.5))
...
richness_dist_test = ggplot(df, aes(x=y)) +
  ...
  geom_density(fill="black", alpha=0.2) +
  theme_gray() +
  labs(title="Richness: Test",  x="Values", y = "Counts") +
  theme(plot.title = element_text(hjust = 0.5))
...
::: {.column width="50%"}
<p><img src="Results/proDij/abundance_dist_train_and_test.png"/></p>

<p><img src="Results/proDij/biomass_dist_train_and_test.png"/></p>

<p><img src="Results/proDij/richness_dist_train_and_test.png"/></p>
:::
:::
...
# ProDij results: total abundance

## Abundance: GLM

```{r reslt_glm_AB_tot, fig.align='center'}
AB_tot_select_glm = read.xlsx("variables_selections/ProDij/AB_tot_select_glm.xlsx")

# Set the order of models
AB_tot_select_glm$models = as.factor(AB_tot_select_glm$models)
AB_tot_select_glm$models <- factor(AB_tot_select_glm$models, levels  = paste0("mod_", 1:nrow(AB_tot_select_glm)))
...
AB_tot_best_model_glm = AB_tot_select_glm[AB_tot_select_glm$models =="mod_7",]
...
# AB_tot_select_glm_long$models <- fct_relevel(AB_tot_select_glm_long$models, c(levels(AB_tot_select_glm$models) ))
...
ggsave("Results/proDij/s_AB_tot_glm.png", plot = g, dpi = 300,height = 5.5,width = 9)

# for (i in predictor_non_r){cat (paste0(i,sep=" + "))}

```

<p align="center">

<img src="Results/proDij/s_AB_tot_glm.png"/>

</p>

**Le meilleur modèle des GLM est :**

$mod_7 = glm(AB_tot ~ Argile + LimonF + SableF + OS, df = data)$

## Abundance: GAM

```{r reslt_gam_AB_tot, fig.align='center'}
AB_tot_select_gam = read.xlsx("variables_selections/ProDij/AB_tot_select_gam.xlsx")
AB_tot_select_gam$models = paste0("mod_", 1:nrow(AB_tot_select_gam))

# Set the order of models
AB_tot_select_gam$models = as.factor(AB_tot_select_gam$models)
AB_tot_select_gam$models <- factor(AB_tot_select_gam$models, levels  = paste0("mod_", 1:nrow(AB_tot_select_gam)))
...
AB_tot_best_model_gam = AB_tot_select_gam[AB_tot_select_gam$models =="mod_5",]
# cat ("The best model based on RMSE is: ")
...
# AB_tot_select_gam_long$models <- fct_relevel(AB_tot_select_gam_long$models, c(levels(AB_tot_select_gam$models) ))
...
ggsave("Results/proDij/s_AB_tot_gam.png", plot = g, dpi = 300,height = 5.5,width = 9)
...
<img src="Results/proDij/s_AB_tot_gam.png"/>

</p>

**Le meilleur modèle des gam est:**

$mod_5 = gam(AB_tot ~ Long + Lat + OS + SableF + Argile + LimonF, df = data)$

## Abundance: RF

```{r reslt_rf_AB_tot, fig.align='center'}

AB_tot_select_rf = read.xlsx("variables_selections/ProDij/AB_tot_select_rf.xlsx")

# Set the order of models
AB_tot_select_rf$models = as.factor(AB_tot_select_rf$models)
AB_tot_select_rf$models <- factor(AB_tot_select_rf$models, levels  = paste0("mod_", 1:nrow(AB_tot_select_rf)))
...
AB_tot_best_model_rf = AB_tot_select_rf[AB_tot_select_rf$models =="mod_6",]
# cat ("The best model based on RMSE is: ")
...
# AB_tot_select_rf_long$models <- fct_relevel(AB_tot_select_rf_long$models, c(levels(AB_tot_select_rf$models) ))
...
ggsave("Results/proDij/s_AB_tot_rf.png", plot = g, dpi = 300,height = 5.5,width = 13)
...
<img src="Results/proDij/s_AB_tot_rf.png"/>

</p>

**Le meilleur modèle des RF est:**

$mod_6 = rf(AB_tot ~ LimonG + Sables + N_tot + Lat + OS + Argile + \\ C.N + SableF + pH_eau + LimonF + Long, df = data)$

## Abundance: GBM

```{r reslt_gbm_AB_tot, fig.align='center'}
AB_tot_select_gbm = read.xlsx("variables_selections/ProDij/AB_tot_select_gbm.xlsx")

# Set the order of models
AB_tot_select_gbm$models = as.factor(AB_tot_select_gbm$models)
AB_tot_select_gbm$models <- factor(AB_tot_select_gbm$models, levels  = paste0("mod_", 1:nrow(AB_tot_select_gbm)))
...
AB_tot_best_model_gbm = AB_tot_select_gbm[AB_tot_select_gbm$models =="mod_11",]
# cat ("The best model based on RMSE is: ")
...
# AB_tot_select_gbm_long$models <- fct_relevel(AB_tot_select_gbm_long$models, c(levels(AB_tot_select_gbm$models) ))
...
ggsave("Results/proDij/s_AB_tot_gbm.png", plot = g, dpi = 300,height = 5.5,width = 13)
...
<img src="Results/proDij/s_AB_tot_gbm.png"/>

</p>

**Le meilleur modèle des GBM est:**

$mod_11 = gbm(AB_tot ~ Lat + N_tot + SableG + Long + OS + LimonF, df = data)$

## Abundance: ANN

```{r reslt_ann_AB_tot, fig.align='center'}

AB_tot_select_ann = read.xlsx("variables_selections/ProDij/AB_tot_select_ann.xlsx")

# Set the order of models
AB_tot_select_ann$models = as.factor(AB_tot_select_ann$models)
AB_tot_select_ann$models <- factor(AB_tot_select_ann$models, levels  = paste0("mod_", 1:nrow(AB_tot_select_ann)))
...
AB_tot_best_model_ann = AB_tot_select_ann[AB_tot_select_ann$models =="mod_12",]
# cat ("The best model based on RMSE is: ")
...
ggsave("Results/proDij/s_AB_tot_ann.png", plot = g, dpi = 300,height = 5.5,width = 13)
...
<img src="Results/proDij/s_AB_tot_ann.png"/>

</p>

**Le meilleur modèle des ANN est:**

$mod_12 = ann(AB_tot ~ Long + C.N + Lat + pH_eau + OS, df = data)$

## Best models for: Abundance
...
$mod = glm(AB_tot ~ Argile + LimonF + SableF + OS, df = data)$

<br/> **GAM**

$mod = gam(AB_tot ~ Long + Lat + OS + SableF + Argile + LimonF, df = data)$

<br/> **RF**

$mod = rf(AB_tot ~ LimonG + Sables + N_tot + Lat + OS + Argile + \\ C.N + SableF + pH_eau + LimonF + Long, df = data)$

<br/> **GBM**

$mod= gbm(AB_tot ~ Lat + N_tot + SableG + Long + OS + LimonF, df = data)$

<br/> **ANN**

$mod = ann(AB_tot ~ Long + C.N + Lat + pH_eau + OS, df = data)$

<br/> <br/> **Conclusion: le meillleur modèle pour l'abondance est le RF**

# ProDij results: total Biomass

## Biomass: GLM

```{r reslt_glm_BM_tot, fig.align='center'}
BM_tot_select_glm = read.xlsx("variables_selections/ProDij/BM_tot_select_glm.xlsx")

# Set the order of models
BM_tot_select_glm$models = as.factor(BM_tot_select_glm$models)
BM_tot_select_glm$models <- factor(BM_tot_select_glm$models, levels  = paste0("mod_", 1:nrow(BM_tot_select_glm)))
...
BM_tot_best_model_glm = BM_tot_select_glm[BM_tot_select_glm$models == "mod_6", ]
# cat ("The best model based on RMSE is: ")
...
# BM_tot_select_glm_long$models <- fct_relevel(BM_tot_select_glm_long$models, c(levels(BM_tot_select_glm$models) ))
...
ggsave("Results/proDij/s_BM_tot_glm.png", plot = g, dpi = 300,height = 5.5,width = 9)
...
<img src="Results/proDij/s_BM_tot_glm.png"/>

</p>

**Le meilleur modèle des GLM est :**

$mod_6 = glm(BM_tot ~ LimonG + LimonF + SableF + C.N + OS, df = data)$

## Biomass: GAM

```{r reslt_gam_BM_tot, fig.align='center'}
BM_tot_select_gam = read.xlsx("variables_selections/ProDij/BM_tot_select_gam.xlsx")
BM_tot_select_gam$models = paste0("mod_", 1:nrow(BM_tot_select_gam))

# Set the order of models
BM_tot_select_gam$models = as.factor(BM_tot_select_gam$models)
BM_tot_select_gam$models <- factor(BM_tot_select_gam$models, levels  = paste0("mod_", 1:nrow(BM_tot_select_gam)))
...
BM_tot_best_model_gam = BM_tot_select_gam[BM_tot_select_gam$models =="mod_5",]
# cat ("The best model based on RMSE is: ")
...
# BM_tot_select_gam_long$models <- fct_relevel(BM_tot_select_gam_long$models, c(levels(BM_tot_select_gam$models) ))
...
ggsave("Results/proDij/s_BM_tot_gam.png", plot = g, dpi = 300,height = 5.5,width = 9)
...
<img src="Results/proDij/s_BM_tot_gam.png"/>

</p>

**Le meilleur modèle des gam est:**

$mod_5 = gam(BM_tot ~ SableF + OS + LimonF + C.N + Lat + Long , df = data)$

## Biomass: RF

```{r reslt_rf_BM_tot, fig.align='center'}

BM_tot_select_rf = read.xlsx("variables_selections/ProDij/BM_tot_select_rf.xlsx")

# Set the order of models
BM_tot_select_rf$models = as.factor(BM_tot_select_rf$models)
BM_tot_select_rf$models <- factor(BM_tot_select_rf$models, levels  = paste0("mod_", 1:nrow(BM_tot_select_rf)))
...
BM_tot_best_model_rf = BM_tot_select_rf[BM_tot_select_rf$models =="mod_14",]
# cat ("The best model based on RMSE is: ")
...
# BM_tot_select_rf_long$models <- fct_relevel(BM_tot_select_rf_long$models, c(levels(BM_tot_select_rf$models) ))
...
ggsave("Results/proDij/s_BM_tot_rf.png", plot = g, dpi = 300,height = 5.5,width = 13)
...
<img src="Results/proDij/s_BM_tot_rf.png"/>

</p>

**Le meilleur modèle des RF est:**

$mod_14 = rf(BM_tot ~ Argiles + LimonF + Long, df = data)$

## Biomass: GBM

```{r reslt_gbm_BM_tot, fig.align='center'}
BM_tot_select_gbm = read.xlsx("variables_selections/ProDij/BM_tot_select_gbm.xlsx")

# Set the order of models
BM_tot_select_gbm$models = as.factor(BM_tot_select_gbm$models)
BM_tot_select_gbm$models <- factor(BM_tot_select_gbm$models, levels  = paste0("mod_", 1:nrow(BM_tot_select_gbm)))
...
BM_tot_best_model_gbm = BM_tot_select_gbm[BM_tot_select_gbm$models =="mod_9",]
# cat ("The best model based on RMSE is: ")
...
# BM_tot_select_gbm_long$models <- fct_relevel(BM_tot_select_gbm_long$models, c(levels(BM_tot_select_gbm$models) ))
...
ggsave("Results/proDij/s_BM_tot_gbm.png", plot = g, dpi = 300,height = 5.5,width = 13)
...
<img src="Results/proDij/s_BM_tot_gbm.png"/>

</p>

**Le meilleur modèle des GBM est:**

$mod_9 = gbm(BM_tot ~ OS + Limons + SableF + N_tot + LimonsF + \\ Lat + C.N + Long, df = data)$

## Biomass: ANN

```{r reslt_ann_BM_tot, fig.align='center'}

BM_tot_select_ann = read.xlsx("variables_selections/ProDij/BM_tot_select_ann.xlsx")

# Set the order of models
BM_tot_select_ann$models = as.factor(BM_tot_select_ann$models)
BM_tot_select_ann$models <- factor(BM_tot_select_ann$models, levels  = paste0("mod_", 1:nrow(BM_tot_select_ann)))
...
BM_tot_best_model_ann = BM_tot_select_ann [BM_tot_select_ann$models =="mod_6",]
# cat ("The best model based on RMSE is: ")
...
# BM_tot_select_ann_long$models <- fct_relevel(BM_tot_select_ann_long$models, c(levels(BM_tot_select_ann$models) ))
...
ggsave("Results/proDij/s_BM_tot_ann.png", plot = g, dpi = 300,height = 5.5,width = 13)
...
<img src="Results/proDij/s_BM_tot_ann.png"/>

</p>

**Le meilleur modèle des ANN est:**

$mod_6 = ann(BM_tot ~ MO + C_tot + SableG + C_org + C.N + Lat + \\ pH_eau + N_tot + Long + SableF + os, df = data)$

## Best models for: Biomass
...
$mod = glm(BM_tot ~ LimonG + LimonF + SableF + C.N + OS, df = data)$

<br/> **GAM**

$mod = gam(BM_tot ~ SableF + OS + LimonF + C.N + Lat + Long , df = data)$

<br/> **RF**

$mod = rf(BM_tot ~ Argiles + LimonF + Long, df = data)$

<br/> **GBM**

$mod= gbm(BM_tot ~ OS + Limons + SableF + N_tot + LimonsF + \\ Lat + C.N + Long, df = data)$

<br/> **ANN**

$mod = ann(BM_tot ~ MO + C_tot + SableG + C_org + C.N + Lat + \\ pH_eau + N_tot + Long + SableF + os, df = data)$

<br/> <br/> **Conclusion: le meillleur modèle pour la biomasse est le RF**

# ProDij results: total Richness

## Richness: GLM

```{r reslt_glm_Richesse, fig.align='center'}
Richesse_select_glm = read.xlsx("variables_selections/ProDij/Richesse_select_glm.xlsx")

# Set the order of models
Richesse_select_glm$models = as.factor(Richesse_select_glm$models)
Richesse_select_glm$models <- factor(Richesse_select_glm$models, levels  = paste0("mod_", 1:nrow(Richesse_select_glm)))
df = Richesse_select_glm
...
# Find the best model based on RMSE
Richesse_best_model_glm <- Richesse_select_glm %>%
  filter(RMSE == min(RMSE))

Richesse_best_model_glm = Richesse_select_glm[Richesse_select_glm$models =="mod_5",]
# cat ("The best model based on RMSE is: ")
...
# Prepare data for facet plotting
Richesse_select_glm_long <- Richesse_select_glm %>%
  pivot_longer(cols = c(R2_adjusted_train, R2_test, RMSE, MAE), 
               names_to = "metric", 
               values_to = "value")

# Richesse_select_glm_long$models <- fct_relevel(Richesse_select_glm_long$models, c(levels(Richesse_select_glm$models) ))

# Plot all metrics with shared x-axis
Richesse_select_glm_long$Response_variables = rep("Richness (nb. sp. /plot)")
g = ggplot(Richesse_select_glm_long, aes(x = models, y = value, fill = Response_variables), color = "black") +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~metric, scales = "free_y", labeller = labeller(
    metric = c(
      R2_adjusted_train = "R² adjusted (train)",
...
ggsave("Results/proDij/s_Richesse_glm.png", plot = g, dpi = 300,height = 5.5,width = 9)
...
<img src="Results/proDij/s_Richesse_glm.png"/>

</p>

**Le meilleur modèle des GLM est :**

$mod_5 = glm(Richesse ~ C.N + Lat + Argile + LimonF + SableF + OS, df = data)$

## Richness: GAM

```{r reslt_gam_Richesse, fig.align='center'}
Richesse_select_gam = read.xlsx("variables_selections/ProDij/Richesse_select_gam.xlsx")
Richesse_select_gam$models = paste0("mod_", 1:nrow(Richesse_select_gam))

# Set the order of models
Richesse_select_gam$models = as.factor(Richesse_select_gam$models)
Richesse_select_gam$models <- factor(Richesse_select_gam$models, levels  = paste0("mod_", 1:nrow(Richesse_select_gam)))

df = Richesse_select_gam
...
# Find the best model based on RMSE
Richesse_best_model_gam <- Richesse_select_gam %>%
  filter(RMSE == min(RMSE))

Richesse_best_model_gam = Richesse_select_gam[Richesse_select_gam$models =="mod_1",]
# cat ("The best model based on RMSE is: ")
# print(Richesse_best_model_gam)


# Prepare data for facet plotting
Richesse_select_gam_long <- Richesse_select_gam %>%
  pivot_longer(cols = c(R2_adjusted_train, R2_test, RMSE, MAE), 
               names_to = "metric", 
               values_to = "value")

# Richesse_select_gam_long$models <- fct_relevel(Richesse_select_gam_long$models, c(levels(Richesse_select_gam$models) ))

# Plot all metrics with shared x-axis
Richesse_select_gam_long$Response_variables = rep("Richness (nb. sp. /plot)")
g = ggplot(Richesse_select_gam_long, aes(x = models, y = value, fill = Response_variables), color = "black") +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~metric, scales = "free_y", labeller = labeller(
    metric = c(
      R2_adjusted_train = "R² adjusted (train)",
...
ggsave("Results/proDij/s_Richesse_gam.png", plot = g, dpi = 300,height = 5.5,width = 9)
...
<img src="Results/proDij/s_Richesse_gam.png"/>

</p>

**Le meilleur modèle des gam est:**

$mod_9 = gam(Richesse ~. , df = data)$

## Richness: RF

```{r reslt_rf_Richesse, fig.align='center'}

Richesse_select_rf = read.xlsx("variables_selections/ProDij/Richesse_select_rf.xlsx")

# Set the order of models
Richesse_select_rf$models = as.factor(Richesse_select_rf$models)
Richesse_select_rf$models <- factor(Richesse_select_rf$models, levels  = paste0("mod_", 1:nrow(Richesse_select_rf)))

df = Richesse_select_rf
...
# Find the best model based on RMSE
Richesse_best_model_rf <- Richesse_select_rf %>%
  filter(RMSE == min(RMSE))

Richesse_best_model_rf = Richesse_select_rf[Richesse_select_rf$models =="mod_2",]
# cat ("The best model based on RMSE is: ")
# print(Richesse_best_model_rf)


# Prepare data for facet plotting
Richesse_select_rf_long <- Richesse_select_rf %>%
  pivot_longer(cols = c(R2_adjusted_train, R2_test, RMSE, MAE), 
               names_to = "metric", 
               values_to = "value")

# Richesse_select_rf_long$models <- fct_relevel(Richesse_select_rf_long$models, c(levels(Richesse_select_rf$models) ))

# Plot all metrics with shared x-axis
Richesse_select_rf_long$Response_variables = rep("Richness (nb. sp. /plot)")
g = ggplot(Richesse_select_rf_long, aes(x = models, y = value, fill = Response_variables), color = "black") +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~metric, scales = "free_y", labeller = labeller(
    metric = c(
      R2_adjusted_train = "R² adjusted (train)",
...
ggsave("Results/proDij/s_Richesse_rf.png", plot = g, dpi = 300,height = 5.5,width = 13)
...
<img src="Results/proDij/s_Richesse_rf.png"/>

</p>

**Le meilleur modèle des RF est:**

$mod_2 = rf(Richesse ~ Long + Lat + SableF + LimonF + LimonG + \\ Argile + C_org + C.N + pH_eau + OS + SableG + \\ Limons + MO + C_tot + N_tot, df = data)$

## Richness: GBM

```{r reslt_gbm_Richesse, fig.align='center'}
Richesse_select_gbm = read.xlsx("variables_selections/ProDij/Richesse_select_gbm.xlsx")

# Set the order of models
Richesse_select_gbm$models = as.factor(Richesse_select_gbm$models)
Richesse_select_gbm$models <- factor(Richesse_select_gbm$models, levels  = paste0("mod_", 1:nrow(Richesse_select_gbm)))

df = Richesse_select_gbm
...
# Find the best model based on RMSE
Richesse_best_model_gbm <- Richesse_select_gbm %>%
  filter(RMSE == min(RMSE))

Richesse_best_model_gbm = Richesse_select_gbm[Richesse_select_gbm$models =="mod_9",]
# cat ("The best model based on RMSE is: ")
...
# Prepare data for facet plotting
Richesse_select_gbm_long <- Richesse_select_gbm %>%
  pivot_longer(cols = c(R2_adjusted_train, R2_test, RMSE, MAE), 
               names_to = "metric", 
               values_to = "value")

# Richesse_select_gbm_long$models <- fct_relevel(Richesse_select_gbm_long$models, c(levels(Richesse_select_gbm$models) ))

# Plot all metrics with shared x-axis
Richesse_select_gbm_long$Response_variables = rep("Richness (nb. sp. /plot)")
g = ggplot(Richesse_select_gbm_long, aes(x = models, y = value, fill = Response_variables), color = "black") +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~metric, scales = "free_y", labeller = labeller(
    metric = c(
      R2_adjusted_train = "R² adjusted (train)",
...
ggsave("Results/proDij/s_Richesse_gbm.png", plot = g, dpi = 300,height = 5.5,width = 13)
...
<img src="Results/proDij/s_Richesse_gbm.png"/>

</p>

**Le meilleur modèle des GBM est:**

$mod_9 = gbm(Richesse ~ pH_eau + Limons + SableG + N_tot + SableF + Lat + OS + Long, df = data)$

## Richness: ANN

```{r reslt_ann_Richesse, fig.align='center'}

Richesse_select_ann = read.xlsx("variables_selections/ProDij/Richesse_select_ann.xlsx")

# Set the order of models
Richesse_select_ann$models = as.factor(Richesse_select_ann$models)
Richesse_select_ann$models <- factor(Richesse_select_ann$models, levels  = paste0("mod_", 1:nrow(Richesse_select_ann)))

df = Richesse_select_ann
...
# Find the best model based on RMSE
Richesse_best_model_ann <- Richesse_select_ann %>%
  filter(RMSE == min(RMSE))

Richesse_best_model_ann = Richesse_select_ann[Richesse_select_ann$models == "mod_12",]
# Richesse_best_model_ann = Richesse_select_ann
# cat ("The best model based on RMSE is: ")
...
# Prepare data for facet plotting
Richesse_select_ann_long <- Richesse_select_ann %>%
  pivot_longer(cols = c(R2_adjusted_train, R2_test, RMSE, MAE), 
               names_to = "metric", 
               values_to = "value")

# Richesse_select_ann_long$models <- fct_relevel(Richesse_select_ann_long$models, c(levels(Richesse_select_ann$models) ))

# Plot all metrics with shared x-axis
Richesse_select_ann_long$Response_variables = rep("Richness (nb. sp. /plot)")
g = ggplot(Richesse_select_ann_long, aes(x = models, y = value, fill = Response_variables), color = "black") +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~metric, scales = "free_y", labeller = labeller(
    metric = c(
      R2_adjusted_train = "R² adjusted (train)",
...
ggsave("Results/proDij/s_Richesse_ann.png", plot = g, dpi = 300,height = 5.5,width = 13)
...
<img src="Results/proDij/s_Richesse_ann.png"/>

</p>

**Le meilleur modèle des ANN est:**

$mod_12 = ann(Richesse ~ Lat + pH_eau + Long + C.N + LimonF, df = data)$

## Best models for: Richness
...
$mod = glm(Richesse ~ C.N + Lat + Argile + LimonF + SableF + OS, df = data)$

<br/> **GAM**

$mod = gam(Richesse ~. , df = data)$

<br/> **RF**

$mod = rf(Richesse ~ Long + Lat + SableF + LimonF + LimonG + \\ Argile + C_org + C.N + pH_eau + OS + SableG + \\ Limons + MO + C_tot + N_tot, df = data)$

<br/> **GBM**

$mod= gbm(Richesse ~ pH_eau + Limons + SableG + N_tot + SableF + Lat + OS + Long, df = data)$

<br/> **ANN**

$mod = ann(Richesse ~ Lat + pH_eau + Long + C.N + LimonF, df = data)$

<br/> <br/> **Conclusion: le meillleur modèle pour la richesse est le RF**
...
## Optimisation/réduction de l'overfitting des bests models

<br/> **\_\_\_\_\_\_\_\_\_\_\_\_\_Avant\_\_\_\_\_\_\_\_\_\_\_\_\_**
...
**\_\_\_\_\_\_\_\_\_\_\_\_\_Après\_\_\_\_\_\_\_\_\_\_\_\_\_**

```{r optimisation AB_tot}
#
...
best_vars_AB_tot = c("LimonG" , "Sables" , "N_tot" , "Lat" , "OS" , "Argile" , "C.N" , "SableF" , "pH_eau" , "LimonF" , "Long")
...
best_vars_BM_tot = c("Argile" , "LimonF" ,"Long")
...
best_vars_Richesse = c("Long" , "Lat" , "SableF" , "LimonF" , "LimonG","Argile" , "C_org" , "C.N" , "pH_eau" , "OS" , "SableG" , "Limons" , "MO" , "C_tot" , "N_tot")

RF_result_Richesse = ForetAlea(var_rep ="Richesse", 
                            df_app= Richesse_train [, c("Richesse", best_vars_Richesse)], 
                            df_valid = Richesse_test [, c("Richesse", best_vars_Richesse)],
                             mtry = 2,
                             ntree= 500,
                             maxnodes = 10)

# summary(RF_result_Richesse$predit)
# summary(Richesse_test$Richesse)
...
AB_tot_df = prodij_explo[,c("AB_tot",best_vars_AB_tot)]
...
BM_tot_df = prodij_explo[,c("BM_tot",best_vars_BM_tot)]
...
best_vars_Richesse = c("C_tot" , "Limons" , "SableF" , "N_tot" , "pH_eau" , "Long")

Richesse_df = prodij_explo[,c("Richesse",best_vars_Richesse)]

Richesse_df = drop_na(Richesse_df)


rf <- gbm(Richesse ~ ., data = Richesse_df)
# rf <- glm(Richesse ~ ., data = Richesse_df, family = 'gaussian')
...
```{r predit Richesse , fig.align='center' , eval=FALSE}
# Prediction avec GLM -----------------------------------------
...
```{r saving best algo1,fig.align='center' , eval=FALSE}
...
AB_tot_df = prodij_explo_non_t[,c("AB_tot",predictor_deep)]
...
```{r Biomass response, fig.align='center', eval=FALSE}
#_______________
BM_tot_df = prodij_explo_non_t[,c("BM_tot",predictor_deep)]
...
```{r Richness response, fig.align='center' , eval=FALSE}
#_______________
Richesse_df = prodij_explo_non_t[,c("Richesse",predictor_deep)]
Richesse_df = drop_na(Richesse_df)
...
**Date de prochain rencontre pour la publication =\> mail (29 juillet 2024)**

**Stratégie de séléction des variables**

```{r}

col1 = c("Long", "Lat" ,"SableF" , "LimonF" ,"LimonG" ,"Argile" ,"C_org" ,"C.N", "pH_eau")

col2 = c(col1,c("Sables","SableG","Limons","MO","C_tot","N_tot"))
vdt_col=c("AB_tot","Richesse")
col_explo = c(vdt_col,col2)


factor_cols <- c("OS", "Ferti_", "w_sol_")

# Extraire les colonnes numériques à explorer
df_explo <- prodij[, col_explo]


# Calcul des statistiques descriptives pour les colonnes numériques
mean_values <- sapply(df_explo, mean, na.rm = TRUE)
stderr_values <- sapply(df_explo, function(x) sd(x, na.rm = TRUE) / sqrt(length(x[!is.na(x)])))
sd_values <- sapply(df_explo, sd, na.rm = TRUE)
var_values <- sapply(df_explo, var, na.rm = TRUE)

# Créer un dataframe de résumé pour les colonnes numériques
summary_df <- data.frame(
  Variables = col_explo,
  Mean = round (mean_values,2),
  Erreur_std = round (stderr_values,2),
  Ecart_yp = round(sd_values,2)
  # variance = round(var_values,2)
)

# Fonction pour calculer les fréquences des colonnes factorielles
factor_summary <- function(col) {
  tbl <- table(prodij[[col]])
  data.frame(
    variable = col,
    level = names(tbl),
...
# Calculer les fréquences pour les colonnes factorielles
factor_summaries <- lapply(factor_cols, factor_summary)

# Combiner toutes les statistiques descriptives dans un seul dataframe
summary_factor_df <- do.call(rbind, factor_summaries)

# Afficher les dataframes de résumé
row.names(summary_df) = NULL
print(summary_df)
# print(summary_factor_df)

# Convertir le dataframe en texte tabulaire
summary_text <- capture.output(print(summary_df, row.names = FALSE))

# Copier le texte tabulaire dans le presse-papiers
writeClipboard(summary_text)
